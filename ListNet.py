...
class ListNet:
	...
	def __init__(self):
		...
		self.sumLabel = 0#文件总行数
		self.feature = []#特征
		self.weight = [2,2,2,2,2,1,1,1,0.5,0.5,0.5]#权重  加入偏置项
		self.label = []#标签
		self.qid = []#查询id
		self.doc_ofQid = {}#每个查询的文档数量
		self.ITER_NUM = 1000#迭代次数
		self.dimention_feature = 11#特征数 偏置项作为1个特征
		self.qid_Num = 0#qid数量
	...
	def ReadExcelFile(self,filepath):
		...
		for i in range(1, sheet.nrows):
			...
			for j in range(0, len(feature_i)):
				if j == 0 or j == 12:
					feature_i[j] = int(feature_i[j])
				else:
					feature_i[j] = round(float(feature_i[j]),5)
			self.feature.append(feature_i[1:12])
			self.label.append(feature_i[12])
			self.qid.append(feature_i[0])
			if feature_i[0] not in self.doc_ofQid:
				self.qid_Num = self.qid_Num + 1
				self.doc_ofQid[feature_i[0]] = 1
			else:
				self.doc_ofQid[feature_i[0]] = self.doc_ofQid[feature_i[0]] + 1
